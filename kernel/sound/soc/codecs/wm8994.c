        ...
        struct snd_ctl_elem_value *ucontrol)
{
	...
	//audio_ctrl_mic_bias_gpio(codec,5);
	...
	if((wm8994->cur_path != path_num) || !(wm8994->codec_state & CALL_ACTIVE))
	{
        ...
        wm8994->prev_path= wm8994->cur_path;
		wm8994->cur_path = path_num;
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	switch(control_data)
	{
		...
		case CMD_CODEC_EMERGENCY_RECOVERY:
			...
			//
			printk("[WM8994] CMD_CODEC_EMERGENCY_RECOVERY\n");
			...
			printk("[WM8994] CMD_CODEC_EMERGENCY_RECOVERY done.\n");
			break;

        case CMD_CODEC_LOOPBACK_TEST : 
            printk("[WM8994] CMD_CODEC_LOOPBACK_TEST\n");
            Earset_Loopback_Chk = 1;
            printk("[WM8994] CMD_CODEC_LOOPBACK_TEST done.\n");
			break;
...
#ifndef CONFIG_SND_ARIANK_WM8994_MASTER
...
void wm8994_shutdown(struct snd_pcm_substream *substream, struct snd_soc_dai *codec_dai)
{
	...
	{		
		...
		wm8994->cur_path = wm8994->prev_path = OFF;
		wm8994_write(codec, WM8994_SOFTWARE_RESET, 0x0000);
		return;
	}
