...
//Rev 04 ~ Rev05
#define YAMAHA_MSENSOR_TRANSFORMATION_REV05    \
{ { 1,   0,   0}, \
  { 0,   1,   0}, \
  { 0 ,  0 ,  1} }
	      ...
	      unsigned int cmd, unsigned long arg)
{
	...
	signed char marray_Rev05[3][3] = YAMAHA_MSENSOR_TRANSFORMATION_REV05;
	...
	switch (cmd) {
		case YAMAHA_IOCTL_GET_MARRAY:
			if(HWREV<=3) //Rev03
			{
				...
				printk(KERN_DEBUG "YAMAHA_MSENSOR_TRANSFORMATION copy OK\n");
			}
			else //Rev05
			{
				if (copy_to_user((void *)arg, marray_Rev05, sizeof(marray))) 
				{
					printk("YAMAHA_MSENSOR_TRANSFORMATION_REV05 copy failed\n");
					return -EFAULT;
				}
				printk(KERN_DEBUG "YAMAHA_MSENSOR_TRANSFORMATION_REV05 copy OK\n");
			}
			break;
