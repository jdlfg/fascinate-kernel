...
static ssize_t gp2asensor_i2c_state_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
	gprintk("called %s \n",__func__);
	...
	i2c_state = gp2a_i2c_err;	

	return sprintf(buf,"%u\n",i2c_state);
}

static int prox_speaker_mode = 0;
static int prox_earjack_mode = 0;

static ssize_t call_proximity_onoff_control_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
	gprintk("called %s \n",__func__);
...
static ssize_t call_proximity_onoff_control_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t size)
{
	int value, i;
	u8 val;
	...
	if(value==1){
		if(!prox_earjack_mode){
			prox_speaker_mode = 0;
			if(!proximity_enable){
				proximity_onoff(1);
				gpio_set_value(GPIO_PS_ON, 1);
				printk(KERN_INFO "[PROX_SENSOR] In Call, Speaker Mode OFF. Proximity Sensor POWER ON \n");
			} else {
				printk(KERN_INFO "[PROX_SENSOR] In Call, Speaker Mode OFF. Proximity Sensor already ON, don't POWER control \n");
			}
		} else {
			prox_speaker_mode = 0;		
			printk(KERN_INFO "[PROX_SENSOR] In Call, Speaker Mode OFF. But 3-pole Earjack Mode already ON \n");						
		}
	} else {
		prox_speaker_mode = 1;	
		if(proximity_enable){
			...
			printk(KERN_INFO "[PROX_SENSOR] In Call, Speaker Mode ON. Proximity Sensor POWER OFF \n");
		} else {
			printk(KERN_INFO "[PROX_SENSOR] In Call, Speaker Mode ON. Proximity Sensor already OFF, don't POWER control \n");
		}
	}
...
static ssize_t call_proximity_onoff_contro_earjack_show(struct device *dev,
        struct device_attribute *attr, char *buf)
{
	gprintk("called %s \n",__func__);
...
static ssize_t call_proximity_onoff_control_earjack_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t size)
{
	int value, i;
	u8 val;
	...
	if(prox_factorymode){
		printk(KERN_INFO "[PROX_SENSOR] Factory TestMode, 3-pole Earjack Mode Event. Don't Prox Sensor pwr control.\n");
		} else {
	if(value==1){
		if(!prox_speaker_mode){
			prox_earjack_mode = 0;
			if(!proximity_enable){
				gpio_set_value(GPIO_PS_ON, 1);			
				proximity_onoff(1);
				printk(KERN_INFO "[PROX_SENSOR] In Call, 3-pole Earjack Mode OFF. Proximity Sensor POWER ON \n");
			}else{
				printk(KERN_INFO "[PROX_SENSOR] In Call, 3-pole Earjack Mode OFF. Proximity Sensor already ON, don't POWER control \n");
			}
		} else{
			prox_earjack_mode = 0;				
			printk(KERN_INFO "[PROX_SENSOR] In Call, 3-pole Earjack Mode OFF. But Speaker Mode already ON \n");			
		}
	} else {
		prox_earjack_mode = 1;	
		if(proximity_enable){
			proximity_onoff(0);
			gpio_set_value(GPIO_PS_ON, 0);		
			printk(KERN_INFO "[PROX_SENSOR] In Call, 3-pole Earjack Mode ON. Proximity Sensor POWER OFF \n");
		} else {
			printk(KERN_INFO "[PROX_SENSOR] In Call, 3-pole Earjack Mode ON. Proximity Sensor already OFF, don't POWER control \n");
		}
	}
	}
	return size;
        ...
        struct device_attribute *attr, const char *buf, size_t size)
{
	...
	printk(KERN_INFO "[PROX_SENSOR] Factory Mode, Proximity Sensor Mode = %d \n",value);

	if(value==1) {
		prox_factorymode = 1;
		printk(KERN_INFO "[PROX_SENSOR] Factory Mode, AlWays ON \n");
	} else {
		prox_factorymode = 0;
		printk(KERN_INFO "[PROX_SENSOR] Factory Mode, AlWays OFF \n");
	}
...
static DEVICE_ATTR(proxsensor_factory_mode,0666, proxsensor_factory_mode_show, proxsensor_factory_mode_store);
static DEVICE_ATTR(proxsensor_file_state,0666, proxsensor_file_state_show, NULL);
static DEVICE_ATTR(gp2asensor_i2c_state,0666, gp2asensor_i2c_state_show, NULL);
static DEVICE_ATTR(call_proximity_onoff,0666, call_proximity_onoff_control_show, call_proximity_onoff_control_store);
static DEVICE_ATTR(call_proximity_onoff_earjack,0666, call_proximity_onoff_contro_earjack_show, call_proximity_onoff_control_earjack_store);
...
static int gp2a_opt_probe( struct platform_device* pdev )
{
  ...
  //  INIT_WORK(&gp2a->work_prox, gp2a_work_func_prox);
	...
	if (device_create_file(switch_cmd_dev, &dev_attr_proxsensor_factory_mode) < 0)
	...
	if (device_create_file(switch_cmd_dev, &dev_attr_gp2asensor_i2c_state) < 0)
		pr_err("Failed to create device file(%s)!\n", dev_attr_gp2asensor_i2c_state.attr.name);

	if (device_create_file(switch_cmd_dev, &dev_attr_call_proximity_onoff) < 0)
		pr_err("Failed to create device file(%s)!\n", dev_attr_call_proximity_onoff.attr.name);

	if (device_create_file(switch_cmd_dev, &dev_attr_call_proximity_onoff_earjack) < 0)
		pr_err("Failed to create device file(%s)!\n", dev_attr_call_proximity_onoff_earjack.attr.name);
...
static int gp2a_opt_resume( struct platform_device* pdev )
{
	...
	if(!prox_speaker_mode && !prox_earjack_mode){
		s3c_gpio_cfgpin(GPIO_PS_ON, S3C_GPIO_OUTPUT);
		gpio_set_value(GPIO_PS_ON, 1);
	} else {
		printk(KERN_INFO "[PROX_SENSOR] In Call, Speaker Mode ON. Proximity not open \n");
	}
	gprintk("[%s] : \n",__func__);
...
static ssize_t proximity_read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	...
	if(value == val_state){
        	...
        	if(value){
			setting = 0x20;
        		opt_i2c_write((u8)(REGS_HYS),&setting);
			gprintk("GP2A Proximity read value changed REGS_HYS = 0x%x  \n", setting);					
        	} else {
			setting = 0x00;        	
        		opt_i2c_write((u8)(REGS_HYS),&setting);     
			gprintk("GP2A Proximity read value changed REGS_HYS = 0x%x  \n", setting);					
