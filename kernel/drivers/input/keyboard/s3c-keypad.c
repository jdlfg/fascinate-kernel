...
#define KEY_KERN_DEBUG	KERN_ERR//KERN_DEBUG
...
static u8 cam_half_on = 0;
static u8 cam_full_on = 0;
...
{
	u32 power_short, home_short, voldn_short, volup_short,cam_half_short, cam_full_short;
	
	printk(KEY_KERN_DEBUG "called %s \n",__func__);

	power_short = ~(readl(S5PV210_GPH2DAT)) & (1 << 6);
	cam_half_short = ~(readl(S5PV210_GPH2DAT)) & (1 << 1);
	cam_full_short = ~(readl(S5PV210_GPH2DAT)) & (1 << 2);
	home_short = ~(readl(S5PV210_GPH3DAT)) & (1 << 0);
	voldn_short = ~(readl(S5PV210_GPH3DAT)) & (1 << 1);
	volup_short = ~(readl(S5PV210_GPH3DAT)) & (1 << 2);
	if(power_short|home_short|voldn_short|volup_short|cam_half_short|cam_full_short)
...
static irqreturn_t s3c_keygpio_pwr_isr(int irq, void *dev_id)
{
	...
	if(key_status){
		if(test_onoff)
		{
			input_report_key(dev,KEYCODE_CLEAR,0);
			printk(KEY_KERN_DEBUG "key Released : clear %d\n",KEYCODE_CLEAR);
		}
		else
		{
			input_report_key(dev,KEYCODE_POWER,0);
		...
		if(test_onoff)
		{
			input_report_key(dev,KEYCODE_CLEAR,1);
...
#endif
			input_report_key(dev,KEYCODE_POWER,1);
...
static irqreturn_t s3c_keygpio_cam_half_isr(int irq, void *dev_id)
{
	unsigned int key_status;
	struct s3c_keypad *pdata = (struct s3c_keypad *)dev_id;
	struct input_dev *dev = pdata->dev;
	...
	key_status = (readl(S5PV210_GPH2DAT)) & (1 << 1);//GPH
	...
	if(key_status){
		...
		cam_half_on = 0;
		input_report_key(dev,KEYCODE_FOCUS,0);
		printk(KEY_KERN_DEBUG "key Released : cam half %d\n",KEYCODE_FOCUS);
	}
	else{
		if(cam_half_on)
			return IRQ_HANDLED;
		...
		cam_half_on = 1;
		input_report_key(dev,KEYCODE_FOCUS,1);
		printk(KEY_KERN_DEBUG "key Pressed : cam half %d\n",KEYCODE_FOCUS);
	}
        return IRQ_HANDLED;
}

static irqreturn_t s3c_keygpio_cam_full_isr(int irq, void *dev_id)
{
	unsigned int key_status;
	struct s3c_keypad *pdata = (struct s3c_keypad *)dev_id;
	struct input_dev *dev = pdata->dev;
	...
	key_status = (readl(S5PV210_GPH2DAT)) & (1 << 2);//GPH
	...
	if(key_status){
		...
		cam_full_on = 0;
		input_report_key(dev,KEYCODE_CAMERA,0);
		printk(KEY_KERN_DEBUG "key Released : cam full %d\n",KEYCODE_CAMERA);
	}
	else{
		if(cam_full_on)
			return IRQ_HANDLED;
		...
		cam_full_on = 1;
		input_report_key(dev,KEYCODE_CAMERA,1);
		printk(KEY_KERN_DEBUG "key Pressed : full %d\n",KEYCODE_CAMERA);
	}
...
static int s3c_keygpio_isr_setup(void *pdev)
{
	...
	set_irq_type(IRQ_EINT(17), IRQ_TYPE_EDGE_BOTH);  
        ret = request_irq(IRQ_EINT(17), s3c_keygpio_cam_half_isr, IRQF_SAMPLE_RANDOM,
                "key cam half", (void *) pdev);
        if (ret) {
                printk(KERN_ERR "request_irq failed (IRQ_KEYPAD (cam half)) !!!\n");
                ret = -EIO;
        }

	s3c_gpio_setpull(S5PV210_GPH2(2), S3C_GPIO_PULL_NONE);
	set_irq_type(IRQ_EINT(18), IRQ_TYPE_EDGE_BOTH);  
        ret = request_irq(IRQ_EINT(18), s3c_keygpio_cam_full_isr, IRQF_SAMPLE_RANDOM,
                "key cam full", (void *) pdev);
        if (ret) {
                printk(KERN_ERR "request_irq failed (IRQ_KEYPAD (cam full)) !!!\n");
                ret = -EIO;
        }
...
static int __init s3c_keypad_probe(struct platform_device *pdev)
{
	...
	set_bit(KEYCODE_FOCUS & KEY_MAX, input_dev->keybit); //for AT cmd test
