...
struct qt602240_data *qt602240 = NULL;
		                                       ...
		                                       //botton_right(fix)    botton_left            center(fix)              top_right(fix)          top_left 
...
static int QT_KEY_ENABLE = 1; // 0 => REV00 touch key unable
...
void qt_Acquisition_Config_Init(void)
{
    ...
    acquisition_config.atchcalsthr = 35;	// 35;
...
void qt_Multitouchscreen_Init(void)
{
	...
	touchscreen_config.blen = 0x20;	// 0x21;

    touchscreen_config.tchthr = 40;	// 40;//45;
    ...
    touchscreen_config.xloclip = 250;
    touchscreen_config.xhiclip = 250;
...
void qt_Noise_Suppression_Config_Init(void)
{
	...
	noise_suppression_config.noisethr = 35;	// 30; 	//35;
...
{
    /* Set CTE config */
    cte_config.ctrl = 0; //sooo.shin_100728 
    cte_config.cmd = 0;	
	if(QT_KEY_ENABLE){
    cte_config.mode = 1;
	}else{
...
int set_tsp_for_ta_detect(int state)
{
	...
	if(state)
		...
		noise_suppression_config.noisethr = 60; 
		noise_suppression_config.ctrl = normal_noise_ctrl; 	
  		 /* Write Noise suppression config to chip. */
	    if (get_object_address(PROCG_NOISESUPPRESSION_T22, 0) != OBJECT_NOT_FOUND)
	    {
	        if (write_noisesuppression_config(0,noise_suppression_config) != CFG_WRITE_OK)
	        {
			/* "Noise suppression config write failed!\n" */
			printk(KERN_DEBUG "\n[TSP][ERROR] line : %d\n", __LINE__);
			ret = WRITE_MEM_FAILED; /* calling function should retry calibration call */
	        }
	    }
	...
	/* Write temporary acquisition config to chip. */
		...
		printk(KERN_DEBUG "\n[TSP][%s] \n", __func__);
		printk(KERN_DEBUG "[TSP] reset acq tchthr=%d, noisethr=%d, noise.ctrl=%d  \n", touchscreen_config.tchthr , noise_suppression_config.noisethr, noise_suppression_config.ctrl);		
		printk(KERN_DEBUG "[TSP] reset acq tchdrift=%d, driftst=%d\n", acquisition_config.tchdrift ,acquisition_config.driftst);
		...
		acquisition_config.driftst = 0; // 4		 
		/* Write temporary acquisition config to chip. */
		if (write_acquisition_config(acquisition_config) != CFG_WRITE_OK)
		{
			/* "Acquisition config write failed!\n" */
...
static u8 is_suspending = 0; // 100628 sooo.shin touchkey_led	
...
//	printk ("%s ###!\n", __func__, __LINE__);
	...
	if(touch_isVoiceCall_state){
		if(!isVoiceCall){
			touch_isVoiceCall_state = 0;

			touchscreen_config.tchthr = 50; // 70 ->60
			...
			acquisition_config.atchcalsthr = 45; // 69 ->59
			acquisition_config.tchdrift = 5; // 4s
			acquisition_config.driftst = 0; // 4					
	...
	if (driver_setup == DRIVER_SETUP_OK)
	{
	...
	#endif
		{
			...
			//20100917 sooo.shin //Median Filter - detection noise
			if(median_on && (quantum_msg[0] == 15)){
				if(quantum_msg[1] & 0x1)	{
					turn_on_median();
				} else {
					printk(KERN_DEBUG "[TSP] TA Noise Detected, and Median filter Not ON, because of quantum_msg[1] = %d\n", quantum_msg[1]);	
				}
			}
...
//#ifdef QT_TOUCHKEY_ENABLE
//			printk(KERN_DEBUG "TSP HW-REV check %d\n", HWREV); 					

			if(QT_KEY_ENABLE){
				if(quantum_msg[0] == 12){
					printk(KERN_DEBUG "atmel_msg[2] = 0x%x   ", quantum_msg[2]);
					if( (quantum_msg[2] & 0x1) && (menu_button==0) ) // menu press
					{
						menu_button = 1;						
						input_report_key(qt602240->input_dev, 158, 1);
							input_sync(qt602240->input_dev);			
						printk(KERN_DEBUG "menu_button is pressed\n");						
					}
					else if( (quantum_msg[2] & 0x2) && (back_button==0) ) // back press
					{
						back_button = 1;			
						input_report_key(qt602240->input_dev, 28, 1);				 
							input_sync(qt602240->input_dev);					
						printk(KERN_DEBUG "back_button is pressed\n"); 			   						
					}
					else if( (~quantum_msg[2] & (0x1)) && menu_button==1 ) // menu_release
					{
						menu_button = 0;				
						input_report_key(qt602240->input_dev, 158, 0);	 
							input_sync(qt602240->input_dev);					
						printk(KERN_DEBUG "menu_button is released\n");										
					}
					else if( (~quantum_msg[2] & (0x2)) && back_button==1 ) // back_release
					{
						back_button = 0;				
						input_report_key(qt602240->input_dev, 28, 0); 
							input_sync(qt602240->input_dev);					
						printk(KERN_DEBUG "back_button is released\n");										
					}
					else
					{
						menu_button=0; 
						back_button=0;
						printk(KERN_DEBUG "Unknow state of touch key\n");
					}
				}
			}
//#endif				
...
static int qt602240_early_suspend(struct early_suspend *h)
{
	...
	is_suspending = 1; // 100628 sooo.shin touchkey_led	
...
static int touchkey_led_cnt = 19;

static int qt602240_late_resume(struct early_suspend *h)
{
	int ret,i;
	unsigned char loop_num;	
	

	ENTER_FUNC;
	is_suspending = 0; // 100628 sooo.shin touchkey_led			
...
//	s3c_gpio_cfgpin(GPIO_TOUCH_INT, S3C_GPIO_SFN(0xf));
	...
	if(IsLDIEnabled())
	{
		//you can control the LED brightness by changing loop_num
		//loop_num=1 : max_brightness        loop_num=31 : min_brightness
		for (loop_num = 0; loop_num < touchkey_led_cnt; loop_num++) {
			gpio_set_value(TOUCHKEY_LED_EN, 0);
			udelay(1);	
...
void init_hw_setting(void)
{
	...
	if (gpio_is_valid(GPIO_TOUCH_EN)) {
		if (gpio_request(GPIO_TOUCH_EN, "GPJ1"))
	...
	if (gpio_is_valid(TOUCHKEY_LED_EN)) {
		if (gpio_request(TOUCHKEY_LED_EN, "GPJ3"))
			printk("Failed to request TOUCHKEY_LED_EN!\n");
		gpio_direction_output(TOUCHKEY_LED_EN, 0);
	}
	is_suspending = 0;	
...
static ssize_t led_control_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned char data;	
	unsigned char loop_num;
	sscanf(buf, "%d", &data);

	 // 100628 sooo.shin touchkey_led	
	if(data && !is_suspending)
	{
		udelay(100);	
		//you can control the LED brightness by changing loop_num
		//loop_num=1 : max_brightness        loop_num=31 : min_brightness
...
static DEVICE_ATTR(LED, S_IRUGO | S_IWUSR | S_IWOTH | S_IXOTH, NULL, led_control_store);
...
static ssize_t qt602240_call_release_touch(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if(isVoiceCall && !touch_isVoiceCall_state){
		...
		printk(KERN_DEBUG "[TSP] Already touch_isVoiceCall_state set, touch_isVoiceCall_state = %d,isVoiceCall = %d \n", isVoiceCall, touch_isVoiceCall_state);
	}
...
void turn_on_median(void)
{
	int ret = WRITE_MEM_OK;	
        ...
        printk(KERN_DEBUG "[TSP] turn_on_median \n");

        noise_suppression_config.ctrl = ta_noise_ctrl; // median filter on
...
//		return 0;
	...
	{
		printk(KERN_DEBUG "%s Skip initialization in LPM Mode.\n",__func__);
		return 0;
	}
#endif
...
//	ts_dev = device_create_drvdata(sec_class, NULL, 0, NULL, "ts");
	...
	if (device_create_file(ts_dev, &dev_attr_LED) < 0)
