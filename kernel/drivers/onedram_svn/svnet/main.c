...
#define DEFAULT_RFS_WAKE_TIME (2*HZ) 
#define DEFAULT_RAW_WAKE_TIME (10*HZ) //Data for LCD On,Change FD time to 8 sec
#define DEFAULT_LCDOFF_RAW_WAKE_TIME (10*HZ) //Data for LCD Off,,Change FD time to 8 sec
...
struct svnet {
	...
	long wake_time; /* jiffies */ /* wake time for rfs packet */
	long wake_process_time; /* jiffies */ /* processing wake time */
	long wake_data_time; /* jiffies */ /* lcd on data wake time */
	long wake_lcdoff_data_time; /* jiffies */ /* lcd on data wake time */
#endif
};
...
static inline void _wake_lock_init(struct svnet *sn)
{
	...
	sn->wake_time = DEFAULT_RFS_WAKE_TIME;
	sn->wake_process_time = DEFAULT_FMT_WAKE_TIME;
	sn->wake_data_time = DEFAULT_RAW_WAKE_TIME;
	sn->wake_lcdoff_data_time = DEFAULT_LCDOFF_RAW_WAKE_TIME;
}
...
static inline void _data_wake_lock_timeout(struct svnet *sn)
{
	wake_lock_timeout(&sn->wlock, sn->wake_data_time);
}

void _data_wakelock_timeout() {
	if (svnet_dev)
		_data_wake_lock_timeout(svnet_dev);
}

static inline void _lcdoff_data_wake_lock_timeout(struct svnet *sn)
{
	wake_lock_timeout(&sn->wlock, sn->wake_lcdoff_data_time);
}

void _lcdoff_data_wakelock_timeout() {
	if (svnet_dev)
		_lcdoff_data_wake_lock_timeout(svnet_dev);
}
...
#define _data_wake_lock_timeout(sn) do { } while(0)
#define _lcdoff_data_wake_lock_timeout(sn) do { } while(0)
#define _non_fmt_wakelock_timeout() do { } while(0)
#define _fmt_wakelock_timeout() do { } while(0)
#define _data_wakelock_timeout() do { } while(0)
#define _lcdoff_data_wakelock_timeout() do { } while(0)
