...
extern void onedram_get_vbase(void** vbase);
...
static int _write(struct sipc *si, int res, struct sk_buff *skb, u32 *mailbox)
{
	...
	case IPCIDX_RAW:
		...
		}else
		{
			...
			{
				_data_wakelock_timeout();
				//printk("lcd off mms tx \n");
			}else
			{
				_lcdoff_data_wakelock_timeout();
				//printk("lcd off tx \n");
			}
		}
		break;
