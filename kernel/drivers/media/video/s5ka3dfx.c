...
#include "s5ka3dfx.h"
#include "s5ka3dfx_lsi_register.h"	//vga tuning
...
extern unsigned int HWREV;		//Arian-K : seperate vga camera power
/*
 * Specification
...
static int s5ka3dfx_power_on(void)
{
	...
	printk(/*KERN_ERR*/ "s5ka3dfx_power_on by s5ka3dfx driver : HWREV %d \n", HWREV);
...
static int s5ka3dfx_power_off(void)
{
	...
	printk(KERN_ERR "s5ka3dfx_power_off by s5ka3dfx driver \n");
...
//Arian-L/K
static int s5ka3dfx_check_connection(struct v4l2_subdev *sd)
{
	int err = 0;
...
static int s5ka3dfx_write_regs(struct v4l2_subdev *sd, u32 regs[], int size)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	...
	for (i = 0; i < size; i++) {
		err = s5ka3dfx_i2c_write(sd, &reg_set[i], sizeof(reg_set[i]));
		if (err < 0)
		{
...
//	struct s5ka3dfx_userset userset = state->userset;
	...
	if(state->vt_mode == 1) 	//s1_camera [For VT camera]
	{
		switch(ctrl->value)
		{
		case EV_MINUS_4:
...
static int s5ka3dfx_check_dataline_stop(struct v4l2_subdev *sd)
{
	...
	for (i = 0; i < reg_test_pattern_index; i++) {
		...
		err = s5ka3dfx_i2c_write(sd, &reg_set, sizeof(reg_set));
		if (err < 0)
		{
			v4l_info(client, "%s: register set failed\n",	__func__);
			return -EIO;
...
//	struct s5ka3dfx_userset userset = state->userset;
	...
	//Arian-L/K 
	case V4L2_CID_CHECK_CAM_VGA:
		printk("Case V4L2_CID_CHECK_CAM_VGA\n");
#if defined (IS_USE_REGISTER_CONFIGURE_FILE_LSI)
		err = 0;
#else
		err = s5ka3dfx_check_connection(sd);
#endif
		break;
