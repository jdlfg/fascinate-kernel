...
#define CE147_KOR_ONLY		// s1 kor only feature
...
#define CMD_SET_EXIF_CTRL				0x07//ykh		//eur merge [eur merge [cts fail]]
...
#define CMD_GPS_TIMESTAMP				0xA7		//eur merge [cts fail]
...
static unsigned char ce147_buf_set_dzoom[13] = {0xff,0xcc,0xaa,0x91,0x7f,0x71,0x65,0x5c,0x54,0x4e,0x48,0x43,0x3f};
...
extern unsigned int HWREV;		//Arian-K only
...
struct ce147_state {
	...
	int pre_af_status;
	int cur_af_status;
	int vt_mode;		//s1_camera [VT call flag]
...
static int condition = 0;		//eur merge [cts fail]
		...
		unsigned char *w_data, unsigned int w_len)
{
	...
	if(ret < 0)
		dev_err(&client->dev, "%s: I2C write ERROR -- FAIL \n", __func__);
...
{	
	...
	printk(/*KERN_ERR*/ "ce147_power_on by ce147 driver : HWREV %d\n", HWREV );

	if(HWREV > 5)		//Arian-L/K
	{
		/* CAM_MEGA_EN - GPJ0(6) */
		...
		if(err) {
			printk(KERN_ERR "failed to request GPJ1(5) for camera control\n");

			return err;
		}

		/* VGA_PWR_EN - GPJ2(5) */
		err = gpio_request(GPIO_VGA_PWR_EN, "GPJ2");
		if (err) {
			printk(KERN_ERR "failed to request GPJ2(5) for camera control\n");
		
			return err;
		}

		/* CAM_VGA_nSTBY - GPJ1(2)	*/
		err = gpio_request(GPIO_CAM_VGA_nSTBY, "GPJ1_2");
		if (err) {
...
#if 1	//Arian-L [ Modify to set pullup/down disable for output gpio ports of camera ]
		if (s3c_gpio_setpull(GPIO_CAM_MEGA_EN, S3C_GPIO_PULL_NONE)) {
			printk(KERN_ERR "failed to set pull GPJ0 for camera control\n");
		}
		
		if (s3c_gpio_setpull(GPIO_CAM_MEGA_nRST, S3C_GPIO_PULL_NONE)) {
			printk(KERN_ERR "failed to set pull GPJ1 for camera control\n");
		}
		
		if (s3c_gpio_setpull(GPIO_VGA_PWR_EN, S3C_GPIO_PULL_NONE)) {
			printk(KERN_ERR "failed to set pull GPJ2(5) for camera control\n");
		}
		...
		// Turn VGA_PWR_EN on
		gpio_direction_output(GPIO_VGA_PWR_EN, 0);
		gpio_set_value(GPIO_VGA_PWR_EN, 1);
		mdelay(1);
	...
	if(err) {
		...
		if(HWREV < 2)	//Arian-K : seperate vga camera power
		{		
			/* CAM_VGA_nSTBY - GPJ1(2)  */
...
#if 1	//Arian-L [ Modify to set pullup/down disable for output gpio ports of camera ]
		if (s3c_gpio_setpull(GPIO_CAM_MEGA_EN, S3C_GPIO_PULL_NONE)) {
			printk(KERN_ERR "failed to set pull GPJ0(6) for camera control\n");
		}
		
		if (s3c_gpio_setpull(GPIO_CAM_MEGA_nRST, S3C_GPIO_PULL_NONE)) {
			printk(KERN_ERR "failed to set pull GPJ1(5) for camera control\n");
		}
...
static int ce147_power_off(void)
{
	...
	printk(KERN_ERR "ce147_power_off by ce147 driver \n");
	...
	if(HWREV > 5)		//Arian-L/K
	{
		...
		// Turn VGA_PWR_EN LOW
		gpio_direction_output(GPIO_VGA_PWR_EN, 1);
		gpio_set_value(GPIO_VGA_PWR_EN, 0);
		mdelay(1);
...
{
	printk(KERN_ERR "ce147_power_en by ce147 driver : onoff %d \n", onoff);
					...
					int timeout, int polling_interval)
{
	...
	while(time_before(jiffies, jiffies_timeout)){
		...
		if(cam_status == value) 
		{
			ce147_msg(&client->dev, "Success : Status check returns %02x\n", cam_status);
			break;
		}
	...
	if(cam_status != value)
	{
		ce147_msg(&client->dev, "TimeOut : Status check returns %02x\n", cam_status);
		return -EBUSY;
	}
	else
...
static int ce147_get_version(struct v4l2_subdev *sd, int object_id, unsigned char version_info[])
{
	...
	ce147_msg(&client->dev, "%s : Enter [id : %d]\n", __func__, object_id);
	...
	case DATA_VERSION_AF:			
		...
		break;
	default:
		dev_err(&client->dev, "%s : failed... There is no case \n", __func__);
		return -EINVAL;
	}
	...
	if(err < 0)
	{
       	dev_err(&client->dev, "%s : failed... i2c error \n", __func__); 	
		return -EIO;
	}
...
static int ce147_get_fw_version(struct v4l2_subdev *sd)
{
	...
	if(err < 0) 
	{
       	printk(KERN_DEBUG "%s : failed... get FW version() \n", __func__); 	
		return  err;
	}
...
static int ce147_get_dateinfo(struct v4l2_subdev *sd)
{
	...
	if(err < 0) 
	{
       	printk(KERN_DEBUG "%s : failed... get DATE information \n", __func__); 	
		return  err;
	}
...
static int ce147_get_sensor_version(struct v4l2_subdev *sd)
{
	...
	if(err < 0) 
	{
       	printk(KERN_DEBUG "%s : failed... get SENSOR version \n", __func__); 	
		return  err;
	}
...
static int ce147_set_preview_size(struct v4l2_subdev *sd)
{
	...
	if(err < 0)
	{
		dev_err(&client->dev, "%s: failed: i2c error .... 2 \n", __func__);
		return -EIO;
	}
...
static int ce147_set_frame_rate(struct v4l2_subdev *sd)
{
	...
	ce147_msg(&client->dev, "%s: Enter => fps : %d \n", __func__, state->fps);
...
static int ce147_set_anti_banding(struct v4l2_subdev *sd)
{
	...
	unsigned char ce147_regbuf_anti_banding[1] = { 0x03 };		//국내향 기준 :60hz 로 변경
	unsigned int ce147_reglen_anti_banding = 1;

	ce147_msg(&client->dev, "%s: Enter -> anti_banding : %d \n", __func__, state->anti_banding);
...
static int ce147_set_dzoom(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	ce147_msg(&client->dev, "%s: Enter : ZoomLevel %d \n", __func__, ctrl->value);
...
static int ce147_set_capture_size(struct v4l2_subdev *sd)
{
	...
	ce147_msg(&client->dev, "%s: Enter : index : %d \n", __func__, index);
...
static int ce147_set_ae_awb(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	ce147_msg(&client->dev, "%s: done => value %d\n", __func__, ctrl->value);
...
static int ce147_set_awb_lock(struct v4l2_subdev *sd, int lock)
{
	...
	ce147_msg(&client->dev, "%s: done => lock %d \n", __func__, lock);
...
static int ce147_set_exif_ctrl(struct v4l2_subdev *sd , int onoff)
{
	...
	printk("%s : thumb_null %d, gps onoff %d \n", __func__, state->thumb_null, onoff);
...
static int ce147_set_capture_exif(struct v4l2_subdev *sd)
{
	...
	struct rtc_time gps_timestamp;//ykh		//eur merge [cts fail]
...
#endif
	unsigned char ce147_str_model[10] = "SHW-M130K\0";
#if 0
	...
	ce147_model_name[1] = strlen(ce147_str_model)+1;

	memcpy(ce147_model_name+2, ce147_str_model, strlen(ce147_str_model)+1);
...
static int ce147_set_jpeg_quality(struct v4l2_subdev *sd)
{
	...
	ce147_msg(&client->dev, "%s: quality = %d\n", __func__, quality);
...
static int ce147_get_snapshot_data(struct v4l2_subdev *sd)
{
	...
	ce147_msg(&client->dev, "%s: Enter : jpeg %d\n", __func__, state->jpeg.enable);
	...
	if(state->jpeg.enable){
		...
		/* Get Thumbnail size */
		if(!state->thumb_null)		//eur merge [cts fail]
		{
...
static int ce147_set_capture_start(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	if(state->jpeg.enable){
		...
		/*
		 * 5. Set JPEG Encoding parameters
		 */
...
static int ce147_set_af_softlanding(struct v4l2_subdev *sd)
{
	...
	ce147_msg(&client->dev, "%s: Enter : runmode = %d \n", __func__, state->runmode);

	if(state->runmode > CE147_RUNMODE_IDLE)
	{
		ce147_msg(&client->dev, "%s: Start \n", __func__);
...
static int ce147_set_flash(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	ce147_msg(&client->dev, "%s: Enter : flash val %d \n", __func__, ctrl->value);
...
static int ce147_set_effect(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	ce147_msg(&client->dev, "%s: Enter : effect %d\n", __func__, ctrl->value);
...
static int ce147_set_saturation(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	ce147_msg(&client->dev, "%s: Enter : saturation %d \n", __func__, ctrl->value);
...
static int ce147_set_wdr(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	ce147_msg(&client->dev, "%s: Enter : wdr %d\n", __func__, ctrl->value);
...
static int ce147_set_anti_shake(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	ce147_msg(&client->dev, "%s: Enter : anti-shake %d\n", __func__, ctrl->value);
...
static int ce147_set_continous_af(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	ce147_msg(&client->dev, "%s: Enter => value %d\n", __func__, ctrl->value);

	if(ctrl->value == CAF_START)		//s1_camera [continuous AF]
	{
...
static int ce147_set_face_detection(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	ce147_msg(&client->dev, "%s: Enter : face_detection %d\n", __func__, ctrl->value);
...
static int ce147_set_focus_mode(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	ce147_msg(&client->dev, "%s: Enter : focus mode %d\n", __func__, ctrl->value);
...
#if 0		//s1_camera [삼성전기 요청사항 : CAF 적용으로 삭제]
...
static int ce147_set_vintage_mode(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	ce147_msg(&client->dev, "%s: Enter : vintage %d\n", __func__, ctrl->value);
...
static int ce147_set_face_beauty(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	ce147_msg(&client->dev, "%s: Enter : beauty shot %d \n", __func__, ctrl->value);
...
static int ce147_set_white_balance(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	ce147_msg(&client->dev, "%s: Enter : wb %d\n", __func__, ctrl->value);
...
static int ce147_set_ev(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	unsigned int ce147_hd_ev_offset = 0x0D;	// 13

	ce147_msg(&client->dev, "%s: Enter : ev %d\n", __func__, ctrl->value);
	...
	{
		ce147_buf_set_ev[1] += ce147_hd_ev_offset;
	}

	dev_dbg(&client->dev, "%s: hd: %d, ev value: 0x%02x\n", __func__, state->hd_preview_on, ce147_buf_set_ev[1]);
...
static int ce147_set_metering(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	ce147_msg(&client->dev, "%s: Enter : metering %d\n", __func__, ctrl->value);
...
static int ce147_set_gamma(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	if(err < 0)
	{
		dev_err(&client->dev, "%s: failed: i2c_write for ce147_set_uv\n", __func__);
		return -EIO;
	}
...
static int ce147_set_face_lock(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	ce147_msg(&client->dev, "%s: Enter : face lock %d\n", __func__, ctrl->value);
...
static int ce147_set_auto_focus(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	ce147_msg(&client->dev, "%s: Enter : ctrl->value %d \n", __func__, ctrl->value);
	...
	{
		dev_err(&client->dev, "%s: Not support cases \n", __func__);
		return 0;
	}
...
static int ce147_get_auto_focus_status(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
	...
	ce147_msg(&client->dev, "%s: Done => status %d \n", __func__, ce147_buf_get_af_status[0]);
	return 0;
}
...
#ifdef CE147_KOR_ONLY		// if ctrl->value is 0xff, do first operation for Factory test.		//main SW Firmware Version
	err = ce147_get_main_sw_fw_version(sd);
	if(err < 0){
		dev_err(&client->dev, "%s: Failed: Reading main sw fw version info from file \n",__func__);
		err = -EIO;
	}
#endif
...
static int ce147_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	ce147_msg(&client->dev, "%s: Enter : value 0x%x \n", __func__, ctrl->id);
...
static int ce147_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	dev_dbg(&client->dev, "%s: ctrl->id 0x%x, runmode %d \n", __func__, ctrl->id, state->runmode);
	...
	case V4L2_CID_CAMERA_BRIGHTNESS:
		//dev_dbg(&client->dev, "%s: V4L2_CID_CAMERA_BRIGHTNESS, runmode %d  \n", __func__, state->runmode);
	...
	case V4L2_CID_CAMERA_ISO:
		//dev_dbg(&client->dev, "%s: V4L2_CID_CAMERA_ISO, runmode %d	\n", __func__, state->runmode);
	...
	case V4L2_CID_CAMERA_METERING:
		//dev_dbg(&client->dev, "%s: V4L2_CID_CAMERA_METERING, runmode %d  \n", __func__, state->runmode);
	...
	case V4L2_CID_CAMERA_RESET:
		ce147_msg(&client->dev, "%s: V4L2_CID_CAMERA_RESET \n", __func__);
		err = ce147_reset(sd);
		break;
	// _]

	case V4L2_CID_CAMERA_VT_MODE:		//s1_camera [VT call flag]
		state->vt_mode = ctrl->value;
		//dev_dbg(&client->dev, "%s: V4L2_CID_CAMERA_VT_MODE : vt_mode %d \n", __func__, state->vt_mode);
		err = 0;
		break;
...
static int ce147_s_ext_ctrl(struct v4l2_subdev *sd, struct v4l2_ext_control *ctrl)
{
	...
	state->exif_ctrl = 0;		//eur merge [cts fail]
