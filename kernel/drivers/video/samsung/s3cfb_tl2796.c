...
#ifdef ELVSS_CONTROL
static bool elvss_enable=true;
struct class *elvss_class;
struct device *switch_elvss_dev;
#endif
...
void tl2796_ldi_disable(void)
{
	...
	// reset ftm_lcd_off to turn lcd on when resume.
	ftm_lcd_off = false;
...
#ifdef ELVSS_CONTROL
void s6e63e0_elvss_onoff(bool onoff)
{
	if(IsLDIEnabled())
	{
		if (onoff == true)
		{
			s6e63m0_panel_send_sequence(s6e63m0_SEQ_ELVSS_ON);
			elvss_enable = true;
		}
		else
		{
			s6e63m0_panel_send_sequence(s6e63m0_SEQ_ELVSS_OFF);
			elvss_enable = false;
		}	
	}
}

static ssize_t elvss_file_cmd_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	gprintk("elvss_show %s\n", buf);
	
	if (elvss_enable == true)
		return sprintf(buf, "%sELVSS ON\n", buf);	
	else
		return sprintf(buf, "%sELVSS OFF\n", buf);

}
static ssize_t elvss_file_cmd_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	gprintk("elvss_store %s\n", buf);
	
	if(strncmp(buf, "ELVSSON", 7) == 0 || strncmp(buf, "elvsson", 7) == 0) {
		gprintk("turn ELVSS on\n");
		s6e63e0_elvss_onoff(true);
	}
	if(strncmp(buf, "ELVSSOFF", 8) == 0 || strncmp(buf, "elvssoff", 8) == 0) {
		gprintk("turn ELVSS off\n");
		s6e63e0_elvss_onoff(false);
	}
		
	return size;
}

static DEVICE_ATTR(elvss_file_cmd,0666, elvss_file_cmd_show, elvss_file_cmd_store);
#endif
...
static int s5p_bl_update_status(struct backlight_device* bd)
{
	...
	if(ftm_lcd_off)
	{
		...
		printk(KERN_DEBUG "ftm_lcd_off is set : gamma bl=%d  gamma_value=%d\n", bl, gamma_value);	
		return 0;
	}
...
#ifdef ELVSS_CONTROL
	elvss_class = class_create(THIS_MODULE, "elvss");
	if (IS_ERR(elvss_class))
		pr_err("Failed to create class(elvss_class)!\n");

	switch_elvss_dev = device_create(elvss_class, NULL, 0, NULL, "switch_elvss");
	if (IS_ERR(switch_elvss_dev))
		pr_err("Failed to create device(switch_elvss_dev)!\n");

	if (device_create_file(switch_elvss_dev, &dev_attr_elvss_file_cmd) < 0)
		pr_err("Failed to create device file(%s)!\n", dev_attr_elvss_file_cmd.attr.name);
#endif
