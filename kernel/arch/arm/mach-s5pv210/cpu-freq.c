...
static unsigned int Protect_1GHz=0;
		       ...
		       unsigned int relation)
{
	...
	if((Protect_1GHz == 1) && (target_freq >= 1000000))
	{
		//printk("Socket B'd don't support 1GHz, just return \n");
		return ret;
	}
...
//	register_early_suspend(&s5pc11x_freq_suspend);	
	...
	if(HWREV == 15)
	{
		printk("Socket B'd Protect 1GHz \n");
		Protect_1GHz = 1;
	}
	
	//Protect_1GHz = 1; //esper, Emul B'd test code
